...
def test_ui_xss_escaping(test_client):
        ...
        with closing(conn.cursor()) as cur:
            cur.execute(
                ...
                INSERT INTO interviews (application_id, scheduled_at, duration_minutes,
                                        mode, location_or_link, status, notes)
                VALUES (
                    %s, %s, 30, 'online', '<img src=x onerror=LOCATION_XSS()>',
                    'scheduled', '<script>RECRUITER_NOTE_XSS()</script>'
                )
                """,
    ...
    assert "<script>RECRUITER_NOTE_XSS()</script>" not in html
    assert "<img src=x onerror=LOCATION_XSS()>" not in html
    assert "&lt;script&gt;RECRUITER_NOTE_XSS()&lt;/script&gt;" in html
    assert "&lt;img src=x onerror=LOCATION_XSS()&gt;" in html
    ...
    assert "RECRUITER_NOTE_XSS()" not in html
    assert "<img src=x onerror=LOCATION_XSS()>" not in html
    assert "&lt;img src=x onerror=LOCATION_XSS()&gt;" in html
...
def test_ui_candidate_withdraw(test_client):
    ...
    assert f"/candidate/withdraw/{app_applied}" in html
    assert f"/candidate/withdraw/{app_shortlisted}" in html

    # Ensure withdraw is absent for others
    assert f"/candidate/withdraw/{app_rejected}" not in html
    assert f"/candidate/withdraw/{app_hired}" not in html
    assert f"/candidate/withdraw/{app_withdrawn}" not in html
