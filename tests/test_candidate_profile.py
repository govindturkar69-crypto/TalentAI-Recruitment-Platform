...
def setup_mock_db(mock_db, db_state=None):
    ...
    def fetchone_se():
        q = last_query.get("q", "")
        if "users" in q:
            return {"role": "candidate", "is_active": True}
        if "candidate_profiles" in q:
            return db_state.get("candidate_profiles")
        if "candidate_education" in q:
            return db_state.get("candidate_education")
        if "candidate_experience" in q:
            return db_state.get("candidate_experience")
        if "candidate_projects" in q:
            return db_state.get("candidate_projects")
        if "candidate_certifications" in q:
            return db_state.get("candidate_certifications")
        if "candidate_achievements" in q:
            return db_state.get("candidate_achievements")
        if "applications" in q:
            return db_state.get("applications", {"cnt": 0})
        if "resumes" in q:
            return db_state.get("resumes")
        return None
...
def test_education_crud(mock_db, auth_client):
    ...
    setup_mock_db(mock_db)
    res = auth_client.post(
        "/candidate/education/add", data={"institution": "MIT", "start_date": "2022-09-01", "end_date": "2018-05-01"}
    )
    assert res.status_code == 302

    setup_mock_db(mock_db, {"candidate_education": None})  # Simulate IDOR failure
...
def test_experience_crud(mock_db, auth_client):
    ...
    res = auth_client.post(
        "/candidate/experience/add",
        data={"company": "Tech", "title": "Dev", "start_date": "2023-01-01", "end_date": "2020-01-01"},
    )
    assert res.status_code == 302
...
def test_projects_crud(mock_db, auth_client):
    setup_mock_db(mock_db)
    res = auth_client.post("/candidate/projects/add", data={"title": "My App", "url": "https://myapp.com"})
    assert res.status_code == 302

    setup_mock_db(mock_db)
    res = auth_client.post("/candidate/projects/add", data={"title": "My App", "url": "javascript:alert(1)"})
    assert res.status_code == 302

    setup_mock_db(mock_db, {"candidate_projects": None})
...
def test_certifications_crud(mock_db, auth_client):
    setup_mock_db(mock_db)
    res = auth_client.post(
        "/candidate/certifications/add", data={"name": "AWS", "issuer": "Amazon", "credential_url": "https://aws.com"}
    )
    assert res.status_code == 302

    setup_mock_db(mock_db, {"candidate_certifications": None})
...
def test_achievements_crud(mock_db, auth_client):
    ...
    setup_mock_db(mock_db, {"candidate_achievements": None})
