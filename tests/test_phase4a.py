...
def setup_data():
    with closing(get_db_connection()) as conn:
        with closing(conn.cursor()) as cur:
            ...
            # Recruiter A
            cur.execute("INSERT INTO users (name, email, password, role) VALUES ('Recruiter A', 'reca_test4a@example.com', 'hash', 'recruiter')")
            reca_id = cur.lastrowid

            # Recruiter B
            cur.execute("INSERT INTO users (name, email, password, role) VALUES ('Recruiter B', 'recb_test4a@example.com', 'hash', 'recruiter')")
            recb_id = cur.lastrowid

            # Candidate
            cur.execute("INSERT INTO users (name, email, password, role) VALUES ('Candidate', 'cand_test4a@example.com', 'hash', 'candidate')")
            cand_id = cur.lastrowid

            # Resume
            cur.execute("INSERT INTO resumes (user_id, resume_path, skills) VALUES (%s, 'dummy.pdf', 'python')", (cand_id,))
            resume_id = cur.lastrowid

            # Job A
            cur.execute("INSERT INTO jobs (recruiter_id, job_title, required_skills) VALUES (%s, 'Job A', 'python')", (reca_id,))
            job_a_id = cur.lastrowid

            # Job B
            cur.execute("INSERT INTO jobs (recruiter_id, job_title, required_skills) VALUES (%s, 'Job B', 'python')", (recb_id,))
...
def create_application(candidate_id, job_id, resume_id, status="applied"):
    with closing(get_db_connection()) as conn:
        with closing(conn.cursor()) as cur:
            cur.execute("INSERT INTO applications (candidate_id, job_id, resume_id, status) VALUES (%s, %s, %s, %s)",
...
def test_bulk_update(test_client):
    ...
    resp = test_client.post("/recruiter/applications/bulk_update", data={
        "selected_apps": [str(app_1), str(app_2), str(app_3), "99999"],
