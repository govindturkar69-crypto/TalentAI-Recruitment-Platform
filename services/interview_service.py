...
def get_recruiter_interviews_for_application(application_id, recruiter_id):
    with closing(get_db_connection()) as conn:
        with closing(conn.cursor()) as cur:
            ...
            cur.execute(
                """
                SELECT i.id, i.application_id, i.scheduled_at, i.duration_minutes, i.mode,
                       i.location_or_link, i.notes, i.status, i.created_at, i.updated_at,
                       CASE
                           WHEN i.status = 'scheduled' AND i.scheduled_at <= NOW()
                           THEN 1
                           ELSE 0
                       END AS can_complete
                FROM interviews i
...
def cancel_future_scheduled_interviews_for_application(cur, application_id):
    ...
    for row in eligible_rows:
        ...
        if cur.rowcount == 1:
            changed_rows.append({"id": row["id"], "scheduled_at": row["scheduled_at"]})
